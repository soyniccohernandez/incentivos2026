<div class="min-h-screen bg-black text-left" x-data="{ showExitModal: false }">
            ...
            <form wire:submit.prevent="guardar" class="space-y-12">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                        <div class="max-w-3xl">
                            ...
                            <p class="text-gray-300 text-sm leading-relaxed font-medium">
                                La información registrada ante la sociedad constituye el <span class="text-white font-bold italic">canal oficial de comunicación</span>. A través de estos medios se enviarán notificaciones legales, cronogramas de convocatoria y resultados finales.
                                <span class="block mt-1 text-gray-400">Si detectas que tu información no está vigente, es imperativo actualizarla de inmediato.</span>
                            </p>
                        </div>
                ...
                <section class="bg-brand-surface border border-brand-border p-8 md:p-12 relative overflow-hidden group">
                    ...
                    <div class="flex items-center gap-6 mb-12">
                        <h2 class="font-bebas text-4xl text-brand-orange tracking-widest uppercase whitespace-nowrap">
                            1. DATOS DEL PROPONENTE
                        </h2>
                    ...
                    <div class="flex flex-col md:flex-row gap-12 items-center md:items-start">
                        ...
                        {{-- Área del Avatar: Circular y a Color --}}
                        <div class="shrink-0">
                            {{-- Usamos la misma lógica de bordes y sombras que en tus otros componentes --}}
                            <div class="w-32 h-32 rounded-full border-2 border-brand-orange shadow-[0_0_25px_rgba(255,77,0,0.2)] flex items-center justify-center overflow-hidden bg-black transition-all duration-500 group-hover:scale-105 mx-auto">
                                ...
                                @elseif(isset($foto_url) && $foto_url) {{-- Por si la variable viene directa --}}
                                <img src="{{ $foto_url }}" class="w-full h-full object-cover">
                                @else
                                <span class="font-bebas text-6xl text-brand-orange">
                                    {{ $iniciales ?? 'SC' }}
                                </span>
                                @endif
                            </div>
                        ...
                        <div class="flex-grow w-full text-left">
                            <div class="grid grid-cols-1 gap-10">
                                ...
                                {{-- Fila Superior: Nombre e Identificación --}}
                                <div class="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-8 items-end border-b border-white/5 pb-6">
                                    <div>
                                        ...
                                        <p class="text-white font-bebas text-5xl md:text-6xl tracking-wide uppercase leading-none">
                                            {{ str_replace(',', '', mb_strtoupper($socio->nombre ?? $nombre)) }}
                                        </p>
                                    </div>
                                    <div class="lg:text-right">
                                        <label class="block text-[10px] uppercase font-black text-gray-500 mb-2 tracking-[5px]">Identificación</label>
                                        <p class="text-white font-mono text-3xl tracking-tighter">
                                            {{ $socio->identificacion ?? $identificacion }}
                                        </p>
                                    </div>
                                </div>

                                {{-- Fila Inferior: CORREO ELECTRÓNICO (Efecto Placa con Relieve) --}}
                                <div class="relative pt-2">
                                    ...
                                    <div class="relative inline-flex">
                                        {{-- Sombra sólida naranja para el efecto de placa --}}
                                        ...
                                        <div class="relative bg-black border border-brand-orange px-8 py-5 flex items-center gap-6 group-hover:-translate-x-1 group-hover:-translate-y-1 transition-all duration-300">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <p class="text-white font-bebas text-3xl md:text-5xl tracking-[3px] lowercase leading-none">
                                                    {{ $socio->correo ?? $correo }}
                                                </p>
                                            </div>

                                            {{-- Icono de arroba decorativo --}}
                ...
                <section class="bg-brand-surface border border-brand-border p-8 md:p-10 space-y-10 text-left">
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="bg-black/40 p-8 border border-brand-border text-left">
                            ...
                            <div x-show="$wire.autoria === 'no'" x-transition class="pt-8 border-t border-white/5" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                                <div class="flex flex-col md:flex-row gap-8 items-end">
                                    <a href="{{ asset('storage/formatos/etapa_01_autorizacion_uso_guion_cia_2026.pdf') }}" target="_blank" class="bg-brand-orange text-black px-6 py-3 font-bold text-[10px] uppercase tracking-widest no-underline shrink-0 hover:bg-white transition-all active:scale-95 shadow-lg">
                                        Descargar Formato Guion
                                    </a>
                                    <div class="flex-1 w-full text-left">
                                        ...
                                        <div class="relative group/input">
                                            <input type="file" wire:model.live="guionArchivo" accept=".pdf" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                ...
                <section class="bg-brand-surface border border-brand-border p-8 md:p-10 space-y-10 text-left">
                    ...
                    <div x-show="$wire.directorPropio === 'no'" x-transition class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-black/40 p-8 border border-dashed border-brand-border mb-10 text-left">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                        ...
                        <div class="p-8 border border-brand-border bg-black/40 group hover:border-brand-orange/40 transition-all duration-500 text-left flex flex-col h-full" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                            ...
                            <div class="space-y-4 text-left">
                                <a href="{{ asset('storage/formatos/'.$doc['formato']) }}" target="_blank" class="block text-center py-2.5 bg-brand-orange text-black font-bold text-[10px] uppercase tracking-widest hover:bg-white transition-all active:scale-95 shadow-lg">
                                    Descargar Formato
                                </a>
                ...
                <section class="bg-brand-surface border border-brand-border p-8 md:p-10 space-y-8 text-left">
                    ...
                    <div class="bg-black/40 p-8 border border-brand-border text-left">
                        ...
                        <div class="flex flex-col md:flex-row gap-8 items-end mb-12 border-b border-white/5 pb-10 text-left" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                            <a href="{{ asset('storage/formatos/formato_declaraciones_2026.pdf') }}" target="_blank" class="bg-brand-orange text-black px-8 py-4 font-bold text-[11px] uppercase tracking-widest no-underline shrink-0 hover:bg-white transition-all active:scale-95 shadow-lg">
                            ...
                            <div class="flex-1 w-full text-left">
                                ...
                                <div class="relative group/input">
                                    <input type="file" wire:model.live="formatoFirmado" accept=".pdf" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                    <div class="bg-black border border-brand-border px-4 py-3 text-[10px] text-gray-400 font-black uppercase tracking-widest flex items-center justify-between group-hover/input:bg-gray-900 transition-colors">
                        ...
                        {{-- SECCIÓN DE CHECKBOXES CON VALIDACIÓN --}}
                        <div class="space-y-8 text-left">
                            <div class="flex flex-col gap-2">
                                <label class="flex items-start gap-5 cursor-pointer group text-left">
                                    ...
                                    <span class="text-sm text-gray-300 group-hover:text-white transition-colors leading-relaxed text-left">
                                ...
                                @error('aceptaTerminos') <span class="text-red-500 text-[10px] ml-11 block uppercase font-bold tracking-widest">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="flex flex-col gap-2">
                                <label class="flex items-start gap-5 cursor-pointer group text-left">
                                    ...
                                    <span class="text-sm text-gray-300 group-hover:text-white transition-colors leading-relaxed text-left">
                                        Autorizo de manera voluntaria, previa, explícita e informada a Actores Sociedad Colombiana de Gestión (ACTORES S.C.G.) para tratar mis datos personales de acuerdo con la Política de Tratamiento de Datos Personales de la Sociedad...
                                    </span>
                                </label>
                                @error('aceptaDatos') <span class="text-red-500 text-[10px] ml-11 block uppercase font-bold tracking-widest">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>
                </section>
    ...
    <div x-show="showExitModal"
        class="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md"
        ...
        <div class="bg-brand-surface border border-brand-border max-w-md w-full p-10 text-center shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden" @click.away="showExitModal = false">
            <div class="mb-8">
                ...
                <p class="text-gray-400 text-sm leading-relaxed uppercase font-bold text-[11px] tracking-[2px]">
                    Si sales ahora, los datos y documentos <span class="text-brand-orange">no se guardarán</span> y perderás el progreso actual.
                </p>
            </div>

            <div class="flex flex-col gap-4">
                <a href="/" class="w-full py-4 bg-red-600 text-white font-bebas text-2xl tracking-widest hover:bg-white hover:text-black transition-all no-underline inline-block">
                    SÍ, SALIR SIN GUARDAR
                </a>
                <button @click="showExitModal = false" class="w-full py-4 bg-transparent border border-brand-border text-gray-500 font-bebas text-2xl tracking-widest hover:border-white hover:text-white transition-all focus:outline-none uppercase">
                    Seguir Editando
                </button>
            </div>
        </div>
