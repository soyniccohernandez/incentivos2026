<div class="min-h-screen bg-black text-left" x-data="{ showExitModal: false }">
    ...
    <nav class="fixed top-0 left-0 w-full z-[1000] flex justify-between items-center px-6 py-5 md:px-12 bg-black/95 border-b border-brand-border backdrop-blur-sm">
        <div class="flex items-center gap-8">
            ...
            <span class="font-bebas text-xl text-gray-500 hidden md:block uppercase tracking-widest border-l border-brand-border pl-8"> Etapa 02: Información Técnica y Elenco </span>
        </div>
            ...
            <form wire:submit.prevent="guardar" class="space-y-12">
                ...
                <section class="bg-brand-surface border border-brand-border p-8 md:p-10 relative overflow-hidden text-left">
                    ...
                    <div class="space-y-10 text-left">
                        ...
                        <div class="bg-black/40 border border-brand-border p-8 md:p-10 relative group hover:border-brand-orange/30 transition-all duration-500" wire:key="miembro-v19-{{ $index }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-x-12 items-start">
                                ...
                                <div class="mb-6 md:mb-0">
                                    <div class="w-28 h-28 rounded-full border-2 {{ $miembro['encontrado'] ? 'border-brand-orange shadow-[0_0_20px_rgba(255,77,0,0.2)]' : 'border-gray-800' }} flex items-center justify-center overflow-hidden bg-black transition-all duration-300 mx-auto">
                                        ...
                                        @else
                                            <svg class="w-12 h-12 text-gray-800" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" /></svg>
                                        @endif
                                    </div>
                                </div>
                                        ...
                                        <div class="flex flex-col md:flex-row gap-6 items-end">
                                            ...
                                            <div class="flex-1 w-full text-left">
                                                ...
                                                <div class="relative group/input">
                                                    <input type="file" wire:model.live="elenco.{{ $index }}.archivo" accept=".pdf" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                                    <div class="bg-black border border-brand-border px-4 py-3 text-[10px] text-gray-400 font-black uppercase tracking-widest flex items-center justify-between group-hover/input:bg-gray-900 transition-colors">
                                                        <span>{{ $miembro['archivo'] ? 'Archivo seleccionado' : 'Seleccionar archivo' }}</span>
                                                ...
                                                @endif
                                                @error("elenco.$index.archivo") <span class="text-red-500 text-[10px] mt-2 block uppercase font-bold text-left">{{ $message }}</span> @enderror
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <section class="bg-brand-surface border border-brand-border p-8 md:p-12 space-y-10 text-left relative overflow-hidden">
                    <div class="absolute top-0 right-0 p-10 opacity-[0.03] pointer-events-none">
                        <svg class="w-40 h-40 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z" /></svg>
                    </div>
                    ...
                    <div class="border-b border-brand-border pb-6">
                        ...
                        <p class="text-[10px] text-gray-500 uppercase font-bold tracking-[3px]"> Los archivos deben ser originales y estar en formato PDF </p>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-10 mt-6">
                        @foreach([
                            ['model' => 'guionFinal', 'label' => 'Guion Versión Final', 'icon' => '<path d="M19 2H5c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V6h10v2z" />', 'desc' => 'Incluir diálogos y encabezados estandarizados.'],
                            ['model' => 'radicadoGuion', 'label' => 'Radicado Guion', 'icon' => '<path d="M11.67 3.87L9.9 2.1 0 12l9.9 9.9 1.77-1.77L3.54 12zM12.33 20.13l1.77 1.77L24 12 14.1 2.1l-1.77 1.77L20.46 12z" />', 'desc' => 'Certificado ante la Dirección de Derecho de Autor.'],
                            ['model' => 'propuestaCreativa', 'label' => 'Propuesta Creativa', 'icon' => '<path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z" />', 'desc' => 'Moodboard, estética visual y de dirección.']
                        ] as $doc)
                        <div class="relative p-8 border border-brand-border bg-black/40 group hover:border-brand-orange/40 transition-all duration-500 text-left flex flex-col h-full" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                ...
                <section class="bg-brand-surface border border-brand-border p-8 md:p-10 space-y-8 text-left">
                    <h2 class="font-bebas text-3xl text-brand-orange border-b border-brand-border pb-4 uppercase tracking-wider text-left"> 3. FINANCIERO Y TIEMPOS </h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                        @foreach([ ['model' => 'presupuesto', 'label' => 'Presupuesto Detallado (XLSX)', 'file' => 'formato_presupuesto_2026.xlsx'], ['model' => 'cronograma', 'label' => 'Cronograma de Producción (XLSX)', 'file' => 'formato_cronograma_2026.xlsx'] ] as $excel)
                        <div class="p-8 border border-brand-border bg-black/40 group hover:border-brand-orange/30 transition-all duration-500 text-left" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                            <label class="block text-xs uppercase font-black text-white tracking-[2px] mb-6">{{ $excel['label'] }}</label>
                            <div class="flex flex-col gap-5 text-left">
                                <a href="{{ asset('storage/formatos/'.$excel['file']) }}" target="_blank" class="block text-center py-3 bg-brand-orange text-black font-bold text-[10px] uppercase tracking-widest hover:bg-white transition-all active:scale-95 shadow-lg"> Descargar Formato </a>
                                <div class="relative group/input">
    ...
    {{-- MODAL DE SALIDA (Consistente con Etapa 1) --}}
    <div x-show="showExitModal" class="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md" x-transition x-cloak>
        <div class="bg-brand-surface border border-brand-border max-w-md w-full p-10 text-center shadow-2xl relative" @click.away="showExitModal = false">
            ...
            <div class="flex flex-col gap-4">
                <a href="/" class="w-full py-4 bg-red-600 text-white font-bebas text-2xl tracking-widest hover:bg-white hover:text-black transition-all no-underline"> SÍ, SALIR </a>
